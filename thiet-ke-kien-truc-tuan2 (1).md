# TÀI LIỆU THIẾT KẾ KIẾN TRÚC — TUẦN 2 (bản 0.4)

**Đề tài:** Bộ nhớ dài hạn cho AI Assistant / LLM Agent — song ngữ Việt · Anh
**Nhóm:** 2 thành viên · Tháng 9–10/2026
**Cập nhật:** 15/09/2026 — thiết kế lại kiến trúc phân lớp, bổ sung xử lý song ngữ

---

## 0. Định vị và nguyên tắc thiết kế

**Mục tiêu:** một hệ bộ nhớ dài hạn hoàn chỉnh, **song ngữ Việt–Anh**, đánh giá đủ chín năng lực.
...
| **9** | **Nhất quán xuyên ngữ** | **tự xây** | **Lớp chuẩn hóa ngôn ngữ + chỉ mục kép** | **5** |

Năng lực 9 là phần riêng của đồ án: lưu bằng tiếng Việt, hỏi bằng tiếng Anh (và ngược lại)
vẫn phải ra đúng. Không benchmark công khai nào chấm việc này.
...
├──────────────────────────────────────────────────────────────────────────┤
│ L3  CHỈ MỤC                                                              │
│     Vector gốc-ngữ │ Vector chuẩn-hóa │ BM25 từ khóa │ Sổ slot           │
├──────────────────────────────────────────────────────────────────────────┤
│ L4  LƯU TRỮ                                                              │
...
| `HybridRetriever` | Dense × 2 chỉ mục + BM25, hợp nhất bằng RRF, rồi rerank | Không (dùng reranker) |
...
Ngôn ngữ là mối quan tâm **xuyên suốt**, không phải một tính năng. Nó chạm vào bốn chỗ.
...
### 3.4. Tầng đọc — chỉ mục kép

Truy vấn được chạy trên **cả hai chỉ mục vector**:

- **Chỉ mục gốc-ngữ** — nhúng `text`. Mạnh khi câu hỏi cùng ngôn ngữ với lúc ghi.
- **Chỉ mục chuẩn-hóa** — nhúng `text_canon`. Mạnh khi câu hỏi khác ngôn ngữ, vì cả hai
  phía đều đã về tiếng Anh, nên không phải dựa vào khả năng bắc cầu của mô hình.

Hai danh sách kết quả hợp nhất bằng RRF. Đây cũng là một thí nghiệm cắt bỏ thành phần sạch:
*chỉ gốc-ngữ* vs *chỉ chuẩn-hóa* vs *cả hai*.
...
### 3.6. Lợi ích kèm theo

LongMemEval, BEAM, FactConsolidation **đều là tiếng Anh**. Hệ song ngữ chạy thẳng được các
benchmark chuẩn, nên số liệu so sánh trực tiếp với các paper — thay vì phải tự dịch và bị hỏi
"số này so với ai".
...
câu hỏi
  ...
  │
  ├─► Dense: chỉ mục gốc-ngữ      ─┐
  ├─► Dense: chỉ mục chuẩn-hóa    ─┼─► RRF ──► ~30 ứng viên
  ├─► BM25 từ khóa                ─┘
  │
...
### Ba trục, áp cho cả chín năng lực
...
| Tính nhất quán | Tỉ lệ dùng đúng fact sau cập nhật · tỉ lệ tự mâu thuẫn · tỉ lệ từ chối đúng lúc · tỉ lệ tôn trọng ràng buộc · **tỉ lệ nhất quán xuyên ngữ** |
| Hiệu năng | p50/p95 tách theo pha (dense · BM25 · rerank · sinh) · token mỗi truy vấn **tách theo ngôn ngữ** · số bản ghi |

### Sáu cấu hình đối chứng
...
| `native-index-only` | chỉ mục chuẩn-hóa | Đóng góp của lớp song ngữ (năng lực 9) |
...
- **Bộ test xuyên ngữ tự xây** — ~60 câu: ghi bằng ngôn ngữ A, hỏi bằng ngôn ngữ B.
...
Kiến trúc trên là bản đầy đủ. Thực thi chia ba mức để không vỡ tiến độ:
...
| **Nên có** (mục tiêu chính) | Chỉ mục kép song ngữ · BM25 + RRF · reranker · `CONFLICT` · `pending` hai phía · ký ức pinned · tóm tắt phiên · `QueryPlanner` | Đây là phần tạo ra đóng góp |
| **Mở rộng** (cắt trước nếu thiếu thời gian) | Bộ test xuyên ngữ · quét tham số 3 mô hình embedding · HTTP API · ký ức procedural | Cắt theo thứ tự này |
...
| 2 | Rà soát benchmark · khung 9 năng lực · kiến trúc phân lớp · chốt công nghệ | — |
...
| 5 | Chỉ mục kép + BM25 + RRF; `QueryPlanner`; khoảng hiệu lực; ký ức pinned | 2, 3, 7, 9 |
| 6 | Reranker; hiệu chỉnh ngưỡng từ chối; tóm tắt phiên; bộ test tiếng Việt & xuyên ngữ | 6, 8 |
| 7 | Chạy 6 cấu hình × 3 trục; phân tích lỗi từ nhật ký vết; tối ưu; hoàn thiện demo | tất cả |
...
| Chín năng lực quá rộng cho 8 tuần | Không năng lực nào đủ sâu | Bảng phạm vi ở mục 9 định sẵn thứ tự cắt |
